...
const OTPFieldSlot: Component<ComponentProps<"div"> & { index: number }> = (props) => {
  ...
  return (
    <div
      class={cn(
        "group border-input relative flex size-10 items-center justify-center border-y border-r text-sm first:rounded-l-md first:border-l last:rounded-r-md",
        local.class
      )}
      {...others}>
      <div
        class={cn(
          ...
          context.activeSlots().includes(local.index) && "ring-ring ring-offset-background ring-2"
        )}
      />
      {char()}
      <Show when={showFakeCaret()}>
        <div class="pointer-events-none absolute inset-0 flex items-center justify-center">
          <div class="bg-foreground h-4 w-px duration-1000" />
        </div>
      </Show>
    </div>
  );
